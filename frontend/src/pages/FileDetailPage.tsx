  ...
  if (!topic || !file) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <Link
          to={routes.topics}
          className="mt-6 inline-block text-sm font-semibold text-slate-700 underline decoration-slate-400 underline-offset-2 hover:text-slate-900"
        >
          Browse topics
        </Link>
      </div>
  ...
  return (
    <div className={cn(theme.light, "relative min-h-full w-full transition-all duration-300")}>
      <div className="relative z-[1] mx-auto max-w-6xl min-w-0 px-4 pt-8 sm:px-6 sm:pt-10">
        ...
        >
          <Link to={routes.home} className="inline-flex shrink-0 items-center gap-1 font-medium hover:opacity-80">
            <Home className="h-4 w-4" strokeWidth={2} />
            Dashboard
          </Link>
          <ChevronRight className="h-4 w-4 shrink-0 text-slate-300" aria-hidden />
          <Link to={routes.topics} className="shrink-0 font-medium hover:opacity-80">
            Topics
          </Link>
          <ChevronRight className="h-4 w-4 shrink-0 text-slate-300" aria-hidden />
          <Link to={topicPath(topic.slug)} className={cn("shrink-0 font-semibold hover:opacity-80", theme.text)}>
            {topic.title}
          </Link>
          <ChevronRight className="h-4 w-4 shrink-0 text-slate-300" aria-hidden />
        ...
        <header
          className={cn(
            "relative mb-8 overflow-hidden rounded-[1.65rem] border-2 bg-white/95 p-6 shadow-sm sm:p-8",
            theme.border,
          )}
        >
          <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className={cn("rounded-md px-2 py-0.5 text-xs font-semibold", theme.chip)}>{file.language}</span>
                <span
              ...
              <h1 className={cn("font-mono text-2xl font-bold tracking-tight sm:text-3xl", theme.text)}>{file.name}</h1>
            ...
            <div className="flex w-full shrink-0 flex-col gap-2 sm:flex-row lg:w-auto">
              <button
                ...
                disabled={!rawCode}
                className={cn(
                  "inline-flex items-center justify-center gap-2 rounded-xl border-2 px-4 py-2.5 text-sm font-semibold shadow-sm transition-all duration-300 disabled:opacity-50",
                  theme.border,
                  theme.light,
                  theme.text,
                )}
              >
                {copied ? (
                  ...
                  <Copy className={cn("h-4 w-4", theme.text)} strokeWidth={2} />
                )}
                {copied ? "Copied" : "Copy code"}
              </button>
              <button
                type="button"
                className="inline-flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-200/90 bg-slate-50/80 px-4 py-2.5 text-sm font-semibold text-slate-500 transition-all duration-300"
        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:items-start">
          <div className="min-w-0 space-y-6 lg:col-span-8">
            ...
            {fetchState.status === "loading" ? (
              <div className={cn("rounded-2xl border-2 bg-white p-10 text-center text-sm text-slate-500", theme.border)}>
                Loading source…
              </div>
            ) : loadError ? (
              ...
              >
                <CodeViewer
                  ...
                  fileName={file.name}
                  headerClassName={cn(theme.accent, "border-b border-white/20 text-white")}
                  outerClassName={cn("border-2 shadow-lg transition-all duration-300", theme.border)}
                />
              </Suspense>
            )}
          </div>
